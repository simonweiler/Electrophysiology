...
[ntsr_cs ntsr_cpncs ntsr_k ntsr_cpnk ...
    ...
    pv_cs pv_cpncs pv_k pv_cpnk vip_k vip_cpnk] = readout_input_strength_callosal(Ephys);
%% 
%NTSR
ntsr_ratios=[];
...
% VIP
vip_ratios=[];
for i=1:size(vip_k,2)
vip_er=[];
vip_er=[(vip_k(:,i)-vip_cpnk(:,i))./(vip_k(:,i)+vip_cpnk(:,i))];
vip_er(find(vip_er==Inf))=NaN;
vip_ratios(:,i)=vip_er;
end

%% combine PENK ans CCi
...
vip_data=[vip_cpnk(:,col) vip_k(:,col)];
...
vip_data2=[vip_cpnk(:,col2) vip_k(:,col2)];
...
%% all individual
fig1= figure;set(fig1, 'Name', 'Barplot groups');set(fig1, 'Position', [400, 200, 600, 600]);set(gcf,'color','w');
subplot(2,2,1)
scatter(ct_data(:,1),ct_data(:,2),20,'k','filled','o');axis square;hold on;rf=refline(1,0); rf.Color= [0 0 0];rf.LineStyle= '--';
xlim([0 450]);ylim([0 450]);
xlabel('Looped CCc (pA)');ylabel('Non-Looped CT (pA)');
text(50,400,['p=' num2str(round(p_ct,2))]);
%text(50,1000,['p=' num2str(round(p_ct,2))]);
subplot(2,2,2)
scatter(cci_data(:,1),cci_data(:,2),20,'k','filled','o');axis square;hold on;rf=refline(1,0); rf.Color= [0 0 0];rf.LineStyle= '--';
xlim([0 250]);ylim([0 250]);
xlabel('Looped CCc (pA)');ylabel('Non-Looped CCi (pA)');
text(50,225,['p=' num2str(round(p_cci,2))]);
%text(50,1000,['p=' num2str(round(p_cci,2))]);
subplot(2,2,3)
scatter(pv_data(:,1),pv_data(:,2),20,'k','filled','o');axis square;hold on;rf=refline(1,0); rf.Color= [0 0 0];rf.LineStyle= '--';
xlim([0 350]);ylim([0 350]);
%xlim([0 1000]);ylim([0 1000]);
xlabel('Looped CCc (pA)');ylabel('Non-Looped PV (pA)');
text(50,300,['p=' num2str(round(p_pv,2))]);
%text(50,1000,['p=' num2str(round(p_pv,2))]);
subplot(2,2,4)
scatter(som_data(:,1),som_data(:,2),20,'k','filled','o');axis square;hold on;rf=refline(1,0); rf.Color= [0 0 0];rf.LineStyle= '--';
xlim([0 350]);ylim([0 350]);
%xlim([0 1000]);ylim([0 1000]);
xlabel('Looped CCc (pA)');ylabel('Non-Looped SST (pA)');
text(50,300,['p=' num2str(round(p_som,2))]);
%text(50,1000,['p=' num2str(round(p_som,2))]);
%% jsut CT
