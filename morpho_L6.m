...
len_br=length(list);
cd(char(morpho_folder));
for m=1:len_br
    dendr(m)=load_tree(list(m).name);
    dendr(m).X=dendr(m).X
    dendr(m).Y=dendr(m).Y
    dendr(m).Z=dendr(m).Z
    X{m}=dendr(m).X;
...
%Combined Features 
r.density=r.total_length/(r.width.*r.height);%P40 single value ; not sure
...
outer_r=max([(r.width-20) (r.height-20)])/5;

%Sholl Analysis 
[s, dd, sd, XP, YP, ZP, iD] = sholl_tree(traces, outer_r, '-s');

%defining the range of the 4 quadrants in x an y
a1x=[0:250]*-1;
a1y=[0:250];
...
r.min_planes_nu=min(com_planes);
r.max_planes_denom=max(com_planes);
