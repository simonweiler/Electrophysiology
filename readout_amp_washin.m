function [epsc ipsc e_i_ratio] = readout_amp(str,cells_idx,stim_type,sol)
...
%3:read out highest freq stimulus (50, short pulses)   
    ...
    for i=1:length(find(cells_idx==1)); 
        ...
        if sol==2
   if max(str(temp(i)).sub_traces_highf(1:1*sr,2))>max(str(temp(i)).sub_traces_highf(1:1*sr,1))==1
